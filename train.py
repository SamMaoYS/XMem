...
for si, stage in enumerate(stages_to_perform):
    ...
    if local_rank == 0:
        ...
        model = XMemTrainer(
            config,
            logger=logger,
            save_path=path.join("saves", long_id, long_id)
        ...
        model = XMemTrainer(
            config, local_rank=local_rank, world_size=world_size
        ).train()
    ...
    if stage != "0":
        change_skip_iter = [
            round(config["iterations"] * f) for f in increase_skip_fraction
        ]
    ...
    try:
        while total_iter < config["iterations"] + config["finetune"]:
            ...
            # Train loop
            model.train()
            for tmp_i, data in enumerate(train_loader):
