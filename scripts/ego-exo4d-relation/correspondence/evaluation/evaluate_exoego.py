    ...
    for object_id in gt["masks"].keys():
        ...
        for exo_cam in EXOCAMS:
            ...
            if (
                ...
                and f"{exo_cam}__{EGOCAM}" in pred["masks"][object_id].keys()
            ):
            ...
            for frame_idx in gt_masks_exo.keys():
                ...
                if gt_obj_exists and pred_obj_exists:
                    ...
                    # # location accuracy
                    location_score = utils.location_score(
                        gt_mask, pred_mask, size=(H, W)
                    )
    ...
    for take_id in preds["results"]:
        ...
        for obj in preds["results"][take_id]["masks"]:
            ...
            for cam in exo_cams:
                try:
                    assert (
                        f"{cam}__{ego_cam}" in preds["results"][take_id]["masks"][obj]
                    )
                except:
                    breakpoint()

                for idx in gt["results"][take_id]["masks"][obj][cam]:
                    assert (
                        idx
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gt-file", type=str, required=True, help="path to json with gt annotations"
    )
    parser.add_argument("--pred-file", type=str, required=True, help="")
